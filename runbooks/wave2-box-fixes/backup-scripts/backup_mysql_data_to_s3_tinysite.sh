...
# - BACKUP_DIR is absolute (was CWD-relative). No credential changes needed:
#   this script never held one (pg trust auth inside the container).
# - Deploy with mode 700 (runbook step).
# NOTE: despite the filename, this dumps POSTGRES (a separate cleanup will
# parameterize + rename; out of scope for this package).
